#include "MySocket.h"
#include <stdexcept>
#include <cstring>
#include <ws2tcpip.h>

// Static Winsock reference counter
int MySocket::s_winsockRefCount = 0;

// ===============================
// Constructor
// ===============================

MySocket::MySocket(SocketType type,
    std::string ip,
    unsigned int port,
    ConnectionType connType,
    unsigned int bufSize)
    : Buffer(nullptr),
    WelcomeSocket(INVALID_SOCKET),
    ConnectionSocket(INVALID_SOCKET),
    mySocket(type),
    connectionType(connType),
    IPAddr(ip),
    Port(static_cast<int>(port)),
    bTCPConnect(false),
    MaxSize(bufSize > 0 ? static_cast<int>(bufSize) : DEFAULT_SIZE)
{
    InitWinsock();

    // Allocate buffer
    Buffer = new char[MaxSize];

    // Zero sockaddr_in
    std::memset(&SvrAddr, 0, sizeof(SvrAddr));
    SvrAddr.sin_family = AF_INET;
    SvrAddr.sin_port = htons(static_cast<u_short>(Port));
    SvrAddr.sin_addr.s_addr = inet_addr(IPAddr.c_str());

    if (mySocket == SERVER)
    {
        if (connectionType == TCP)
        {
            SetupServer();
        }
        else // UDP server
        {
            SetupUDPServer();
        }
    }
    else
    {
        // CLIENT
        if (connectionType == UDP)
        {
            // UDP client: create socket now
            ConnectionSocket = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
            if (ConnectionSocket == INVALID_SOCKET)
            {
                throw std::runtime_error("Failed to create UDP client socket");
            }
        }
        // TCP client: socket created in ConnectTCP()
    }
}

// ===============================
// Destructor
// ===============================

MySocket::~MySocket()
{
    if (ConnectionSocket != INVALID_SOCKET)
    {
        closesocket(ConnectionSocket);
        ConnectionSocket = INVALID_SOCKET;
    }

    if (WelcomeSocket != INVALID_SOCKET)
    {
        closesocket(WelcomeSocket);
        WelcomeSocket = INVALID_SOCKET;
    }

    if (Buffer != nullptr)
    {
        delete[] Buffer;
        Buffer = nullptr;
    }

    CleanupWinsock();
}

// ===============================
// Winsock helpers
// ===============================

void MySocket::InitWinsock()
{
    if (s_winsockRefCount == 0)
    {
        WSADATA wsaData;
        int result = WSAStartup(MAKEWORD(2, 2), &wsaData);
        if (result != 0)
        {
            throw std::runtime_error("WSAStartup failed");
        }
    }
    ++s_winsockRefCount;
}

void MySocket::CleanupWinsock()
{
    --s_winsockRefCount;
    if (s_winsockRefCount == 0)
    {
        WSACleanup();
    }
}

// ===============================
// Server setup
// ===============================

void MySocket::SetupServer()
{
    WelcomeSocket = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    if (WelcomeSocket == INVALID_SOCKET)
    {
        throw std::runtime_error("Failed to create TCP welcome socket");
    }

    if (bind(WelcomeSocket, reinterpret_cast<sockaddr*>(&SvrAddr), sizeof(SvrAddr)) == SOCKET_ERROR)
    {
        throw std::runtime_error("bind() failed on server");
    }

    if (listen(WelcomeSocket, SOMAXCONN) == SOCKET_ERROR)
    {
        throw std::runtime_error("listen() failed on server");
    }
}

void MySocket::SetupUDPServer()
{
    ConnectionSocket = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
    if (ConnectionSocket == INVALID_SOCKET)
    {
        throw std::runtime_error("Failed to create UDP server socket");
    }

    if (bind(ConnectionSocket, reinterpret_cast<sockaddr*>(&SvrAddr), sizeof(SvrAddr)) == SOCKET_ERROR)
    {
        throw std::runtime_error("bind() failed on UDP server");
    }
}

// ===============================
// Connect / Disconnect (TCP)
// ===============================

void MySocket::ConnectTCP()
{
    if (connectionType != TCP || mySocket != CLIENT)
        return;

    if (bTCPConnect)
        return;

    ConnectionSocket = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    if (ConnectionSocket == INVALID_SOCKET)
    {
        throw std::runtime_error("Failed to create TCP client socket");
    }

    if (connect(ConnectionSocket,
        reinterpret_cast<sockaddr*>(&SvrAddr),
        sizeof(SvrAddr)) == SOCKET_ERROR)
    {
        closesocket(ConnectionSocket);
        ConnectionSocket = INVALID_SOCKET;
        throw std::runtime_error("connect() failed");
    }

    bTCPConnect = true;
}

void MySocket::DisconnectTCP()
{
    if (connectionType != TCP)
        return;

    if (ConnectionSocket != INVALID_SOCKET)
    {
        shutdown(ConnectionSocket, SD_BOTH);
        closesocket(ConnectionSocket);
        ConnectionSocket = INVALID_SOCKET;
    }

    bTCPConnect = false;
}

// ===============================
// Send / Receive
// ===============================

void MySocket::SendData(const char* data, int size)
{
    if (data == nullptr || size <= 0)
        return;

    if (connectionType == TCP)
    {
        if (mySocket == SERVER)
        {
            // Ensure we have an accepted connection
            if (ConnectionSocket == INVALID_SOCKET)
            {
                // Accept a client if not already done
                sockaddr_in clientAddr;
                int addrLen = sizeof(clientAddr);
                ConnectionSocket = accept(WelcomeSocket,
                    reinterpret_cast<sockaddr*>(&clientAddr),
                    &addrLen);
                if (ConnectionSocket == INVALID_SOCKET)
                {
                    throw std::runtime_error("accept() failed in SendData");
                }
            }
        }
        else
        {
            // CLIENT: must have connected
            if (!bTCPConnect || ConnectionSocket == INVALID_SOCKET)
                throw std::runtime_error("TCP client not connected");
        }

        int sent = send(ConnectionSocket, data, size, 0);
        if (sent == SOCKET_ERROR)
        {
            throw std::runtime_error("send() failed");
        }
    }
    else // UDP
    {
        int sent = sendto(ConnectionSocket,
            data,
            size,
            0,
            reinterpret_cast<sockaddr*>(&SvrAddr),
            sizeof(SvrAddr));
        if (sent == SOCKET_ERROR)
        {
            throw std::runtime_error("sendto() failed");
        }
    }
}

int MySocket::GetData(char* destBuffer)
{
    if (destBuffer == nullptr)
        return 0;

    int received = 0;

    if (connectionType == TCP)
    {
        if (mySocket == SERVER)
        {
            // Accept if not already connected
            if (ConnectionSocket == INVALID_SOCKET)
            {
                sockaddr_in clientAddr;
                int addrLen = sizeof(clientAddr);
                ConnectionSocket = accept(WelcomeSocket,
                    reinterpret_cast<sockaddr*>(&clientAddr),
                    &addrLen);
                if (ConnectionSocket == INVALID_SOCKET)
                {
                    throw std::runtime_error("accept() failed in GetData");
                }
            }
        }
        else
        {
            if (!bTCPConnect || ConnectionSocket == INVALID_SOCKET)
                throw std::runtime_error("TCP client not connected");
        }

        received = recv(ConnectionSocket, Buffer, MaxSize, 0);
        if (received == SOCKET_ERROR)
        {
            throw std::runtime_error("recv() failed");
        }
    }
    else // UDP
    {
        sockaddr_in fromAddr;
        int fromLen = sizeof(fromAddr);
        received = recvfrom(ConnectionSocket,
            Buffer,
            MaxSize,
            0,
            reinterpret_cast<sockaddr*>(&fromAddr),
            &fromLen);
        if (received == SOCKET_ERROR)
        {
            throw std::runtime_error("recvfrom() failed");
        }
    }

    if (received > 0)
    {
        std::memcpy(destBuffer, Buffer, received);
    }

    return received;
}

// ===============================
// Getters / Setters
// ===============================

std::string MySocket::GetIPAddr() const
{
    return IPAddr;
}

void MySocket::SetIPAddr(const std::string& ip)
{
    // Prevent change if TCP connection already established
    if (connectionType == TCP && bTCPConnect)
        throw std::runtime_error("Cannot change IP after TCP connection established");

    IPAddr = ip;
    SvrAddr.sin_addr.s_addr = inet_addr(IPAddr.c_str());
}

void MySocket::SetPort(int port)
{
    if (connectionType == TCP && bTCPConnect)
        throw std::runtime_error("Cannot change port after TCP connection established");

    Port = port;
    SvrAddr.sin_port = htons(static_cast<u_short>(Port));
}

int MySocket::GetPort() const
{
    return Port;
}

SocketType MySocket::GetType() const
{
    return mySocket;
}

void MySocket::SetType(SocketType type)
{
    // Prevent change if TCP server already listening or client connected
    if (connectionType == TCP &&
        (WelcomeSocket != INVALID_SOCKET || bTCPConnect))
    {
        throw std::runtime_error("Cannot change SocketType while TCP is active");
    }

    mySocket = type;
}
